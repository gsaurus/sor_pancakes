...
public class AnimFrame{
    ...
    protected static AnimFrame readTypesTreeAndZero(RandomAccessFile rom, long address) throws IOException
  {
    ...
    if (check == 149){
      ...
      int a0 = rom.readByte();
      rom.skipBytes(1);int a1 = rom.readByte();
      rom.skipBytes(1);int a2 = rom.readByte();
    ...
    public static AnimFrame read(RandomAccessFile rom, long address, int type) throws IOException{
        if (type == 3) return readTypesTreeAndZero(rom, address);
        if (type == 2) return readTypeTwo(rom, address);
        if (type == 1) return readTypesOneAndZero(rom, address);
        if (type == 0) return readTypeZero(rom, address);
        return null;
    }
